...
const projects = [
  ...
  },
  {
    ...
    description: "Remote physical therapy platform with real-time MQTT sensor data, progress tracking, and video-guided exercises.",
    tags: ["Flutter", "MQTT", "Python", "FastAPI"],
    accent: "emerald",
    appStore: false,
  },
  {
...
const contact = {
  ...
  linkedin: "linkedin.com/in/yelynnhtet",
...
// ── Profile inline SVG placeholder ────────────────────────────────────

function ProfilePlaceholder() {
  return (
    <div className="relative h-64 w-64 shrink-0 sm:h-80 sm:w-80" aria-label="Profile photo placeholder">
      <svg
        viewBox="0 0 320 320"
        ...
        aria-hidden="true"
      >
        {/* Abstract geometric portrait — indigo + slate */}
        <rect width="320" height="320" rx="16" fill="#1e1b4b" />
        <circle cx="160" cy="120" r="64" fill="#312e81" opacity="0.6" />
        <rect x="96" y="200" width="128" height="80" rx="40" fill="#3730a3" opacity="0.5" />
        <circle cx="160" cy="280" r="80" fill="#4338ca" opacity="0.25" />
        {/* Subtle grid lines */}
        {Array.from({ length: 5 }).map((_, i) => (
          <line key={`h${i}`} x1="0" y1={64 + i * 48} x2="320" y2={64 + i * 48} stroke="#6366f1" strokeWidth="0.5" opacity="0.12" />
        ))}
      </svg>
      {/* Overlay label */}
      <div className="pointer-events-none absolute inset-0 flex items-end justify-center pb-4">
        <span className="rounded-full bg-slate-950/60 px-3 py-1 text-xs text-slate-400 backdrop-blur-sm">
          + Add Photo
        </span>
      </div>
    </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 flex gap-3 pt-1">
                    {project.appStore ? (
                      <span className="inline-flex items-center gap-1.5 rounded-lg border border-slate-700 px-3 py-1.5 text-xs font-medium text-slate-400 transition-colors hover:border-slate-500 hover:text-slate-300">
        ...
        <footer id="contact" className="scroll-mt-20 space-y-8 rounded-2xl border border-slate-800 bg-slate-900/60 p-8 sm:p-12">
          <div className="space-y-3">
            ...
            <p className="max-w-md text-sm leading-relaxed text-slate-400">
              I&rsquo;m based in Tokyo and open to remote opportunities worldwide. Reach out — I&rsquo;d love to hear about your project.
            </p>
          </div>
